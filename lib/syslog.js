...
async function ems_poll() {
	for (let i=0;i < own_states.length;i++){
		if (own_states[i].polling && !unloaded) {
            ...
            var url = emsesp + "/api/system/send " ;
        
			try {var response = await ems_put(url,telegram);}
...
async function syslog_server() {
	...
	server.on("msg", data => {
        ...
        alive = false;
        adapter.getForeignState(aliveState,function (err, state) { 
            if (state != null) alive  = state.val;
            if (!unloaded) sys(data);
            else {adapter.log.info("syslog close due adapter stop");server.close();return;}	
        });
...
async function sys (data) {
    ...
    if (true) {	
        ...
        if (!unloaded) adapter.setState("syslog.telegram.telegram_raw",{ack: true, val: data.msg});
        if (!unloaded)adapter.setState("syslog.telegram.src",{ack: true, val:src});
        if (!unloaded)adapter.setState("syslog.telegram.dest",{ack: true, val:dest});
        if (!unloaded)adapter.setState("syslog.telegram.type",{ack: true, val:type});
        if (!unloaded)adapter.setState("syslog.telegram.type_text",{ack: true, val:typet});
        if (!unloaded)adapter.setState("syslog.telegram.type_raw",{ack: true, val:typer});
        if (!unloaded)adapter.setState("syslog.telegram.offset",{ack: true, val:offset});
        if (!unloaded)adapter.setState("syslog.telegram.data",{ack: true, val:tdata});
...
async function write_undefinedstate(src,typer,offset,tdata) {
	...
	for (let i = 0;i< d.length;i++) {
		...
		try {let dec = parseInt(d[i],16);adapter.setState(statename, {ack: true, val: dec});}
