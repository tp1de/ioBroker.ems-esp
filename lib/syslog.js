...
async function write_ownstate(statename,value,own) {
	...
	if (own.states !== "") {try {obj.common.states = JSON.parse(own.states);} catch(e) {}}
	...
	obj.common.name= "telegram type:"+own.type + " offset:" + own.offset + " bytes:" + own.bytes + " - state:" + own.state_type;
...
async function post(id,telegram) {
	...
	try {
		const response = await ems_put(url,telegram);
