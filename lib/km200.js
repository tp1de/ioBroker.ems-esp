...
async function km200_test() {
	try {
		...
		if (data == undefined || data == "") {
			adapter.log.error("error reading km200 gateway information (wrong passwords) - stop km200 read"); 
			return(false);
		}
	} catch(error) {
...
async function recsw(field,d) {
	if (db.substring(0,3) == "sql" ) {
		...
		let src = await getsource(db);
		let id = await getid(field,db);
		if (id ==0) {
			try{await adapter.setStateA(stateid, {ack: true, ts: 1000, val: 0});} catch(e){}
			let id = await getid(field,db);
		}
...
function getsource(db) {
	return new Promise(function(resolve) {
        var query = "select id from iobroker.sources where name = 'system.adapter.ems-esp." + adapter.instance +"';";
...
function getid(field,db) {
	return new Promise(function(resolve) {
        var query = 'select id from iobroker.datapoints where name = "' + field + '";';
        //console.log(query);
