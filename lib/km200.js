// Function module for km200 / ip-inside web-api access  
//
//
//
...
const init = async function(a,u,i) {
	...
	try {
		let o = await adapter.getForeignObjectAsync("system.config");
    ...
    else if (adapter.config.km200_active === true) {
		...
		if (result[i].ems_field == "" && result[i].km200 != "") {
			...
			if (body != undefined) {
				try {
					...
					await adapter.setStateChangedAsync(result[i].km200, {ack: true, val: val});
				}
				catch(error) {
...
async function recs() {
    ...
    if (recordings === true && adapter.config.km200_active === true) {
...
async function sleep(ms) {
	alive = false;
	state = await adapter.getForeignStateAsync(aliveState);
	if (state != null) alive  = state.val;
	if (alive) {let timer;await sleep_(ms);clearTimeout(timer);return;}
	else return;	
	async function sleep_(ms) {return new Promise(resolve => timer=setTimeout(resolve, ms));}
}

module.exports ={init,state_change};