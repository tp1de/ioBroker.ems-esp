...
async function km200_get(url) {
	...
	if (b.status != 200) {await sleep(500);b = await axios(options);}
	...
	if (b.status == 200) {
		try {
			...
			return(data);
		} catch(decrypt) {data = " ";adapter.log.error("km200 get: "+km200_server+" - "+decrypt);}
	}
}
...
function km200_decrypt(input) {
	...
	try {
		...
		output = JSON.parse(s);
	} catch(d) {output = null;adapter.log.error("km200 decrypt: "+km200_server+" - "+d);}
	return output;
}
