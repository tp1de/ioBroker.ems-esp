...
const init = async function (a, u, i) {
    ...
    if (active) {
        ...
        if (adapter.config.recordings && !unloaded) {
            ...
            i.recordings = setInterval(function () {
            ...
            if (o != undefined) {
                try {
                    ...
                    if (o.type != 'yRecording') {
                        await adapter.setObjectAsync(obj1._id, obj1);
                    } else {
...
async function initrecs(r) {
    for (let i = 1; i < r.length; i++) {
        if (r[i].type == 'yRecording') {
            ...
            if (db.trim() != '') {
                await adapter.setObjectAsync(`${r[i].km200}._Hours`, {
                    type: 'state',
                    common: {
                ...
                await adapter.setObjectAsync(`${r[i].km200}._Days`, {
                    type: 'state',
                    common: {
                ...
                await adapter.setObjectAsync(`${r[i].km200}._Months`, {
                    type: 'state',
                    common: {
