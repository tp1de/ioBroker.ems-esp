...
async function initrecs(r) {
	...
	for (let i=1; i < r.length; i++) {
		if (r[i].type == "yRecording" ) {
			...
			if (db.trim() != "") {
				await adapter.setObjectAsync(r[i].km200+"._Hours",{type: "state",common: {name: "db hourly recordings",type: "number", read: true, write: true, unit: uom}, native: {}});
				await adapter.setObjectAsync(r[i].km200+"._Days",{type: "state",common: {name: "db daily recordings",type: "number", read: true, write: true, unit: uom}, native: {}});
				await adapter.setObjectAsync(r[i].km200+"._Months",{type: "state",common: {name: "db monthly recordings",type: "number", read: true, write: true, unit: uom}, native: {}});
			}

			await adapter.setObjectAsync(r[i].km200+".Hours",{type: "state",common: {name: "recordings hours",type: "json", read: true, write: true, unit: uom}, native: {}});
			await adapter.setObjectAsync(r[i].km200+".Days",{type: "state",common: {name: "recordings days",type: "json", read: true, write: true, unit: uom}, native: {}});
			await adapter.setObjectAsync(r[i].km200+".Months",{type: "state",common: {name: "recordings months",type: "json", read: true, write: true, unit: uom}, native: {}});
		}
	}
