...
async function recsw(field,d,t) {
	...
	for (let i = 0; i < d.length;i++){if (d[i].state.val === null || d[i].state.val == "null") d[i].state.val = 0;}
	...
	if (db.substring(0,3) == "sql" ) {
		...
		for (let i = 0; i < d.length;i++){
			let values = "", command = "";
			if (! unloaded && d[i].state.val > 0) {
				values = "("+id+","+d[i].state.val+","+d[i].state.ts+",1,"+src+",0)";
				command = "INSERT INTO iobroker.ts_number (id, val, ts, ack, _from, q) VALUES "+ values;
		...
		if (data != " ") {
			...
			try {
				for (let ii = 0; ii < data.recording.length; ii++){
					...
					if (i == 0 && ma == ii+1 && r.uom != "C" && r.uom != "°C") {
						multi = 1;
