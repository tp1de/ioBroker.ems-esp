				...
				try {
					...
					if (body.type == "floatValue") {
						if (body.minValue != undefined && body.maxValue != undefined && body.state == undefined) {
							if (val < body.minValue || val > body.maxValue) {
								adapter.log.info("km200: "+result[i].km200+" value out of range: "+val);
								val = "invalid";
							}
						}
					}
...
async function recsw(field,d,t) {
	...
	if (db.substring(0,8) == "influxdb" ) {
		...
		for (let i = 0; i < d.length;i++){
			if (!unloaded ) {try {adapter.sendTo(db,"storeState", d[i]);} catch(e) {}}
		}
	}
...
function km200_obj(n,o) {
	...
	try {
		if (o.state != undefined){
			...
			// This does not work - Admin Gui collapses
			//let st = JSON.stringify(o.state).replace("}{",",");
			//c.common.states = JSON.parse(st);
		}
	} catch(e) {}
...
const state_change = async function (id,state,obj) {
	if (unloaded) return;
	let value = state.val;
	adapter.log.info("KM200 write change: "+ id + ": "+value);
	try {
