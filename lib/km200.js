...
const init = async function(a,u,i) {
	...
	if (db == "") {
		adapter.log.info("no database instance selected for recordings");

	}
	else {
		const state = await adapter.getForeignStateAsync("system.adapter."+db+".connected");	
		if (state == undefined) {
			adapter.log.warn("database instance "+db+ "for recordings not existing");db = ""; 
		}  
		else if ( state.val == false) {
			adapter.log.warn("database instance "+db+" for recordings not active");db = ""; 
		}
	}
...
async function km200_test() {
	try {
		...
		adapter.log.error("error reading km200 gateway information (wrong ip address) - stop km200 read");
		return(false);
	}
	return(true);
}
...
async function recsw(field,d,t) {
	if (db.substring(0,3) == "sql" ) {
		...
		if (id == 0) {
			try{await adapter.setStateAsync(field, {ack: true, ts: 1000, val: 0});} catch(e){}
			const id = await getid(field,db);
		}
		...
		let command = "INSERT INTO iobroker.ts_number (id, val, ts, ack, _from, q) VALUES "+ values;
		command += "ON DUPLICATE KEY UPDATE val=values(val)" +";";
		adapter.sendTo(db,"query", command);
	...
	let s = v.sort(SortArray);	
		
	let ss = [],sss = [];
	for (let i = 0; i < s.length;i++){
		let date = new Date(s[i].ts);
...
async function hours(r) {
	...
	for (let i=0;i<3;i++) {
		...
		catch(error) {adapter.log.error("error"+data);data = " "; }
		if (data != " ") {
...
async function initrecs1(r) {

	await adapter.setObjectNotExistsAsync("recordings.created",{type: "state",common: {type: "boolean", read: true, write: true}, native: {}});
	for (let i=1; i < r.length; i++) {
		if (r[i].type == "yRecording" ) {
			let obj = await adapter.getObjectAsync(r[i].km200);
			const f = obj.native.km200.recordedResource.id.substring(1).split("/").join(".");
			let uom = "";
			r[i].uom = "";
			try {
				let obj = await adapter.getObjectAsync(r[i].km200);
				...
				adapter.log.info("error reading recording reference: "+f+ " for: "+r[i].km200);
			}

			await adapter.setObjectNotExistsAsync(r[i].km200+"._Hours",{type: "state",common: {name: "db entries for history",type: "number", read: true, write: true, unit: uom}, native: {}});
			await adapter.setObjectNotExistsAsync(r[i].km200+"._Days",{type: "state",common: {name: "db entries for history",type: "number", read: true, write: true, unit: uom}, native: {}});
			await adapter.setObjectNotExistsAsync(r[i].km200+"._Months",{type: "state",common: {name: "db entries for history",type: "number", read: true, write: true, unit: uom}, native: {}});

		}
	}

	adapter.getState("recordings.created", function(err, state) {
		if(state == null || state.val === false) {
			...
			adapter.setState("recordings.created", {ack: true, val: true});
		}
	});
}
...
async function initrecs(r) {
	...
	for (let i=1; i < r.length; i++) {
		if (r[i].type == "yRecording" ) {
			...
			await adapter.setObjectNotExistsAsync(r[i].km200+".Hours",{type: "state",common: {name: "recordings hours",type: "array", read: true, write: true, unit: uom}, native: {}});
			await adapter.setObjectNotExistsAsync(r[i].km200+".Days",{type: "state",common: {name: "recordings days",type: "array", read: true, write: true, unit: uom}, native: {}});
			await adapter.setObjectNotExistsAsync(r[i].km200+".Months",{type: "state",common: {name: "recordings months",type: "array", read: true, write: true, unit: uom}, native: {}});
...
async function enable_state(stateid,retention,interval) {
	...
	let obj = await adapter.getObjectAsync(stateid);
	try {
...
function getsource(db) {
	return new Promise(function(resolve) {
		const query = "select id from iobroker.sources where name = 'system.adapter.ems-esp." + adapter.instance +"';";
...
function getid(field,db) {
	return new Promise(function(resolve) {
		const query = 'select id from iobroker.datapoints where name = "' + field + '";';
