...
async function recsw(field,d) {
	if (db.substring(0,3) == "sql" ) {
		...
		adapter.sendTo(db, "deleteAll", {id: field}, function (result) {
			try {adapter.sendTo(db,"storeState", d);}
			catch(e) {}
		});
...
function enable_state(stateid,retention,interval) {
	...
	adapter.sendTo(db, "enableHistory", {id: id, options:
		...
		if (result.success) {adapter.setState(stateid, {ack: true, ts: 1000, val: 0});}
	});
}
