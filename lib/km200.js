...
const init = async function(a,u,i) {
	...
	if (db == "") {
		...
		} catch(e) {adapter.log.error("can't read database name");database = "iobroker";}
	}
		...
		if (r.km200 !== "") {
			...
			if (o != undefined) {
				try {
					...
					if (o.type != "yRecording") adapter.setState(r.km200, {ack: true, val: val});
					else if (adapter.config.recordings) adapter.setState(r.km200, {ack: true, val: val});
				}
				catch (error) {
		...
		if (result[i].km200 != "" && result[i].type != "yRecording" ) {
			...
			if (body != undefined) {
				try {
					...
					if (!unloaded) await adapter.setStateAsync(result[i].km200, {ack: true, val: val});
				}
				catch(error) {
...
async function recsw(field,d,t) {
	...
	if (db.substring(0,3) == "sql" ) {
		...
		if (id == 0) {
			adapter.log.info("recordings first init: " + field);
...
async function hours(r) {
	...
	for (let i=0;i<3;i++) {
		...
		catch(error) {adapter.log.error("error reading recordings on hour"+data);data = " "; }
		if (data != " ") {
...
async function days(r) {
	...
	for (let i=0;i<3;i++) {
		...
		try {data = await km200_get(url1);}
		catch(error) {adapter.log.error("error reading recordings on days"+data);data = " "; }
		if (data != " ") {
...
async function months(r) {
	...
	for (let i=0;i<3;i++) {
		...
		try {data = await km200_get(url1);}
		catch(error) {adapter.log.error("error reading recordings on months"+data);data = " "; }
		if (data != " ") {
	...
	for (let i=1; i < r.length; i++) {
		if (r[i].type == "yRecording" ) {
			...
			try {
				...
				adapter.log.debug("can't read recordings reference object: "+f);
			}
			try { uom = obj.native.km200.unitOfMeasure;} catch(e) {
				adapter.log.debug("can't read uom of reference object: "+f);
			}
			...
			if (db.trim() != "") {
				await adapter.setObjectNotExistsAsync(r[i].km200+"._Hours",{type: "state",common: {name: "db hourly recordings",type: "number", read: true, write: true, unit: uom}, native: {}});
				await adapter.setObjectNotExistsAsync(r[i].km200+"._Days",{type: "state",common: {name: "db daily recordings",type: "number", read: true, write: true, unit: uom}, native: {}});
				await adapter.setObjectNotExistsAsync(r[i].km200+"._Months",{type: "state",common: {name: "db monthly recordings",type: "number", read: true, write: true, unit: uom}, native: {}});
			}
...
async function enable_state(stateid,retention,interval) {
	...
	try {
		...
		function (result) {
			if (result.error) { adapter.log.error("enable state error: " + stateid+ "  " + result.error);}
			if (result.success) {}
		});
		//}
	} catch(e) {}
