		...
		if (result[i].km200 != "" && result[i].type != "yRecording" ) {
			...
			if (body != undefined) {
				try {
					...
					if (body.type == "errorList" && o.values != undefined){val = JSON.stringify(body.values);}
					if (body.type == "systeminfo" && o.values != undefined){val = JSON.stringify(body.values);}
					if (body.type == "floatValue") {
						if (body.minValue != undefined && body.maxValue != undefined && body.state == undefined) {
							if (val < body.minValue || val > body.maxValue) {
								adapter.log.debug("km200: "+result[i].km200+" value out of range: "+val);
								val = "invalid";
							}
						}
					}
