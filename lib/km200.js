...
async function recsw(field,d) {
	if (db.substring(0,3) == "sql" ) {
		...
		await sleep(2000);
		if (!unload) adapter.sendTo(db,"storeState", d);
	}
	else if (db.substring(0,8) == "influxdb" ) {
		for (let i = 0; i < d.length;i++){
			adapter.sendTo(db,"storeState", d[i]);
		}
	}
}
...
async function hours() {
		...
		if (data != " ") {
			...
			for (let ii = 0; ii < data.recording.length; ii++){
				if (data.recording[ii] !== null){
					...
					daten.push({id: field,state: {ts: + ts ,val: wert}});
				}
			}
		}
	...
	//adapter.log.info(JSON.stringify(daten));
