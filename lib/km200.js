...
async function km200_get_old(url) {return new Promise(function(resolve,reject) {
	const urls = km200_server +  "/" + url.split(".").join("/") ;
	const options =
        {   url: urls,
        	method: "GET",
...
async function km200_put_old(url,value,type) {return new Promise(function(resolve,reject) {
	if (unloaded) return;
	let data;
	switch (type) {
		case "switchProgram":
