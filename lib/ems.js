...
async function read_energy() {
	...
	await energy(db, "energy.actualPower.power", "energy.actualPower._Hours",end,intervall,step);
	await energy(db, "energy.actualCHPower.power", "energy.actualCHPower._Hours",end,intervall,step);
	await energy(db, "energy.actualDHWPower.power", "energy.actualDHWPower._Hours",end,intervall,step);
	...
	await energy(db, "energy.actualPower.power", "energy.actualPower._Days",end,intervall,step);
	await energy(db, "energy.actualCHPower.power", "energy.actualCHPower._Days",end,intervall,step);
	await energy(db, "energy.actualDHWPower.power", "energy.actualDHWPower._Days",end,intervall,step);
	...
	for (let i=0;i<12;i++) {
		...
		await energy(db, "energy.actualPower.power", "energy.actualPower._Months",end,intervall,step);
		await energy(db, "energy.actualCHPower.power", "energy.actualCHPower._Months",end,intervall,step);
		await energy(db, "energy.actualDHWPower.power", "energy.actualDHWPower._Months",end,intervall,step);
...
async function energy(db,idr,idw,end,intervall,step) {
	...
	if (res.length > 1) {
		...
		recs.push({ts: ts ,val: 0 ,ack: true});
	}
	...
	let field1 = idw.replace(/_Days/g, "Days");
