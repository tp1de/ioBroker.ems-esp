...
const init = async function(a,i) {
	...
	try {
		...
		/*
		if (enum_format != 1 && enum_format != 2  ) {
			adapter.log.error("wrong formatting options for api/mqtt enum format in ems-esp settings -> correct: Value or Index ");
			adapter.setState("info.connection_ems", false, true);
			return;
		}
		*/
	...
	if (vs != -1) {
		adapter.log.warn("*** Version 3.7. might change structure and names of some warm water / dhw objects");
