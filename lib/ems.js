...
const init = async function(a,o,i) {
	...
	try {
		...
		ems_version = "V2";
		adapter.log.error("API version V2 identified or EMS-ESP Gateway IP Adress wrong. V2 is not supported anymore");
		adapter.log.info("last adapter version supporting V2 was v1.15.0");
		adapter.log.info("Within shell: cd /opt/iobroker");
		adapter.log.info("iobroker install ems-esp@1.15.0   or if already installed: iobroker upgrade ems-esp@1.15.0");
		return;
	}

	try {ems_version = data["System Info"].version;} catch(e) {}
