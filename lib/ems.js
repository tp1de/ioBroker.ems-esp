...
const init = async function(a,o,i) {
	...
	if (version == "V2") await v2_readwrite(); // test for API V2 if states are writable

	adapter.log.info("ems  :"+adapter.config.emsesp_active + " " + ems_polling + " secs");
	i.ems = setInterval(function() {ems_read(version);}, ems_polling*1000);
...
async function write_state(statename,value,def) {
	...
	*/
	try {
		let state = await adapter.getStateAsync(statename1);
