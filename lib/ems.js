...
const init = async function(a,i) {
	...
	try {
		...
		if (bool_format != 6 ) {
			adapter.log.error("wrong api bool format in ems-esp settings");
			adapter.setState("info.connection_ems", false, true);
			return;
		}
		if (enum_format != 1 ) {
			adapter.log.error("wrong api enum format in ems-esp settings");
			adapter.setState("info.connection_ems", false, true);
			return;
		}

		adapter.log.info("format options form ems-esp checked --> OK" );
	} catch(e) {
