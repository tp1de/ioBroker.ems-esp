...
async function init_states_emsesp(version) {
	...
	if (version == "V3") {
		url = emsesp +  "/api/system/settings";
		try {settings = await ems_get(url);}
		catch(error) {settings = "";}
	}
	await sleep(ems_http_wait);
	if (settings != "") read_status(settings,"esp.settings");
	if (data != "Invalid") {
					...
					if (typeof value2 !== "object") {
						...
						for (const [key3, value3] of Object.entries(wert2)) {
							if (key1 == "Devices" || key1 == "devices") {
								let key2a = wert2.type + " " + key2;
								write_status(entry+"."+key1+"."+key2a+"."+key3,value3);
								let pos = wert2.name.indexOf("DeviceID") + 9;
								id = wert2.name.substr(pos,4);
