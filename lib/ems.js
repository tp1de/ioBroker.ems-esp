	...
	if (adapter.config.ems_dallas) {
		...
		if (data != "Invalid") {
			...
			for (const [key, value] of Object.entries(sensors)) {
				let url1 = emsesp +  "/api/dallassensor/" + key;
				try {data = await ems_get(url1); }
				catch(error) {data = "Invalid";}
				await sleep(ems_http_wait);
				if (data != "Invalid") {
					try {value1 = JSON.parse(data);}
					catch(error) {}
					write_sensor("dallas."+value1.id,value.temp,value1);
				}
			}
		}
	}
	...
	if (adapter.config.ems_analog) {
		...
		if (data != "Invalid") {
			...
			for (const [key, value] of Object.entries(analogs)) {
				let url1 = emsesp +  "/api/analogsensor/" + key;
				try {data = await ems_get(url1); }
				catch(error) {data = "Invalid";}
				await sleep(ems_http_wait);
				if (data != "Invalid") {
					...
					write_sensor("analog.gpio"+value1.gpio,value1.value,value1);
				}
			}
		}
	}
