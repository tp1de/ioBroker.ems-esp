...
const init = async function(a,i) {
	...
	if (!unloaded && adapter.config.ems_energy) {
		...
		if (ems_polling != 15) {
			ems_polling = 15;
			adapter.log.info("set ems-polling to 15 seconds due to energy statistics");
	...
	if (!unloaded) i.ems_energy = setInterval(function() {read_energy();}, 600 * 1000); // every 10 minutes
...
async function energy(db,idr,idw,end,intervall,step) {
	...
	try {await adapter.sendToAsync(db,"storeState", recs);} catch(e) {}
}
