...
async function init_energy() {
	...
	await adapter.setObjectAsync(statename,{type: "state", common: {type: "number", name: "ems: energy consumption", unit: "kWh", role: "value", read: true, write: false}, native: {}});
...
async function read_energy() {
	...
	for (let i=0;i<12;i++) {
		...
		await energy(db, "energy.actualPower.power", "energy.actualPower._Months",end,intervall,step,"mm");
		await energy(db, "energy.actualCHPower.power", "energy.actualCHPower._Months",end,intervall,step,"mm");
		await energy(db, "energy.actualDHWPower.power", "energy.actualDHWPower._Months",end,intervall,step,"mm");
