...
async function energy(db,idr,idw,end,intervall,step,t) {
	...
	if (db.substring(0,3) == "sql" ) {
		...
		const database = "iobroker";

		if (id == 0) try {await adapter.sendToAsync(db,"storeState", {id: adapt+idw, state: recs});} catch(e) {}
		else {
	...
	} else try {await adapter.sendToAsync(db,"storeState", {id: adapt+idw, state: recs});} catch(e) {}

	//const start =  (end-intervall);
	//if (t == "mm") try {await adapter.sendToAsync(db,"deleteAll",{id: adapt+idw});} catch(e) {adapter.log.error(e);}
	//else try {await adapter.sendToAsync(db,"deleteRange",[{id: adapt+idw, start: start, end: end}]);} catch(e) {adapter.log.error(e);}
	//await sleep(500);
