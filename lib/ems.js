...
async function init_states_emsesp(version) {
	...
	if (data != "Invalid") {
		...
		try {status = JSON.parse(data).Status;}catch(e){adapter.log.error("error reading ems Status");}
		try {
			system = JSON.parse(data).System;
			adapter.log.info("ems-esp-version identified:" + system.version);		
		}catch(e){adapter.log.error("error reading ems system info");}
		...
		for (let i=0; i < devices.length; i++) {
			//if (devices[i].handlers != undefined || devices[i].handlers_received != undefined) {
			if (true) {
