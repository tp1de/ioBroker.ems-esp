...
const init = async function (a, i) {
    ...
    try {
        ...
        try {
            ...
            adapter.log.error('EMS-ESP Gateway connection error');
            return;
        }
    }
...
async function ems_read(version) {
            ...
            for (let i = 0; i < devices.length; i++) {
                if (device_check(devices[i])) {
                    ...
                    try {
                        ...
                        adapter.log.error(`EMS-ESP http read polling error:${url1}`);
                    }
                }
                await adapter.delay(ems_http_wait);
            }
    ...
    if (adapter.config.ems_dallas) {
        if (last_version.substring(0, 3) < '3.6') {
            ...
            try {
                ...
                adapter.log.error(`error reading dallassensors:  ${error}`);
                data = 'Invalid';
            }
            await adapter.delay(ems_http_wait);
        ...
        if (sensors > 0) {
            ...
            try {
                ...
                data = 'Invalid';
                adapter.log.error(`error reading temperaturesensors:  ${error}`);
            }
            await adapter.delay(ems_http_wait);
            ...
            if (data != 'Invalid') {
                ...
                try {
                    ...
                    adapter.log.error(`error parsing temperaturesensors:  ${error}`);
                }
    ...
    if (adapter.config.ems_analog) {
        ...
        if (data != 'Invalid') {
            ...
            try {
                ...
                adapter.log.error(`error reading analogsensors:  ${error}`);
            }
    ...
    if (adapter.config.ems_custom_elements) {
        ...
        try {
            ...
            adapter.log.error(`error reading custom entities:  ${error}`);
            data = 'Invalid';
        }
        await adapter.delay(ems_http_wait);
        ...
        if (data != 'Invalid') {
            ...
            for (const [key, value] of Object.entries(custom)) {
                ...
                await adapter.delay(ems_http_wait * 4);
