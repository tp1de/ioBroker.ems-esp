...
async function init_states_emsesp(version) {
	...
	if (data != "Invalid") {
		...
		try {devices = JSON.parse(data).Devices; devices_count = devices.length; }catch(e){adapter.log.error("error reading ems devices");}
...
async function write_state(statename,value,def) {
	if (!unloaded) {
		...
		try {defj = JSON.parse(def);} 
			...
			adapter.log.warning("wrong state definition: " + statename + "  " + def);
			def = "";
		}
