...
//		adapter.log.warn("new ems-esp firmware version .... instance will restart after 60 seconds");
//		adapter.log.warn("please stop adapter instance next time before uploading new ems-esp firmware");
//		try {clearInterval(i.ems_energy);} catch(e) {}
//		try {clearInterval(i.ems);} catch(e) {}
//		try {clearInterval(i.poll);} catch(e) {}
//		adapter.delay (60000);
//		adapter.restart();
//	}
	...
	if (data != "Invalid") {
		...
		try {devices = JSON.parse(data).Devices;}catch(e){}
		try {devices = JSON.parse(data).devices;}catch(e){}
		try {
			adapter.setState("info.connection_ems", true, true);
		}
