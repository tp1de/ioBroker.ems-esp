...
const init = async function(a,i) {
	...
	if (devs != -1) {
		adapter.log.warn("*** You are using a non-productive version of ems-esp gateway firmware");
		adapter.log.warn("*** There might be structural changes involved. Use is on your own risk");
	}
	if (vs != -1) {
		adapter.log.warn("*** Version 3.7. will change structure and names of some warm water / dhw objects");
		adapter.log.warn("*** you need to adjust your scripts / vis ui's to reflect these changes if necessary");
		adapter.log.warn("*** You need to set parameter to delete states on start (once)");
		adapter.log.warn("*** If dhw-states have db/history entries you need to delete by yourself using object browser");
	}
