...
async function energy(db,idr,idw,end,intervall,step) {
	...
	try {await adapter.sendToAsync(db,"deleteRange",[{id: idw, start: start, end: end}]);} catch(e) {adapter.log.error(e);}
	try {await adapter.sendToAsync(db,"storeState", {id: idw, state: recs});} catch(e) {adapter.log.error(e);}
}
