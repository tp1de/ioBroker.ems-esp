		...
		if (data != "Invalid") {
			...
			for (const [key, value] of Object.entries(custom)) {
				...
				if (data != "Invalid") {
					...
					write_sensor(type+".c_"+def.name,def.value,def);
				}
			}
		}
	}
...
async function energy(db,idr,idw,end,intervall,step,t) {
	...
	if (t != "mm") {
		...
		for (let i=0;i<14;i++) {
			...
			if (result.result.length > 1) res.push(result.result[1]);
			else {
	...
	if (res.length > 0) {
		for (let i=0; i< res.length;i++) {
			...
			//if (t == "hh") adapter.log.info(h+":"+m);
			if (t == "hh" && m == 30) recs.push({ts: ts ,val: val,ack: true});
			else if (t != "hh") recs.push({ts: ts ,val: val,ack: true});
		}
	} else {
		//const ts = end - intervall/2;
	...
	if (db.substring(0,3) == "sql" ) {
		...
		if (id == 0) try {
			...
			//adapter.log.info("ems: first db init "+adapt+idw);
		} catch(e) {}
		else {
		...
		/*
		try {
			let recsd = await adapter.sendToAsync(db,"getHistory", {id: adapt+idw, options: {start:0, end: Date.now() + 3600000, addId: true }});
			let resd = recsd.result;

			//adapter.log.info(JSON.stringify(recsd));

			for (let ii = 0; ii< resd.length;ii++) {
				adapter.log.info(JSON.stringify(resd[ii]));
				adapter.sendTo(db,"delete", [{id: resd[ii].id, state: {ts: resd[ii].ts}}], function(result) {
		...
		let id = adapt+idw;
		adapter.sendTo(db,'deleteAll', [{id: id}], function(result) {
			adapter.log.info(JSON.stringify(result));
		});
