...
async function init_states_emsesp(version) {
	...
	if (data != "Invalid") {
		...
		try {devices = JSON.parse(data).Devices; devices_count = devices.length; }catch(e){}
		try {devices = JSON.parse(data).devices; devices_count = devices.length; }catch(e){}
...
async function ems_read(version) {
	...
	if (last_version != actual_version) {
		...
		adapter.log.warn("new ems-esp firmware version .... instance will restart after 10 seconds");
