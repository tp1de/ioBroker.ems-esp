...
const init = async function (a, i) {
    ...
    try {
        ...
        try {
            data = await ems_get(url);
            ems_version = 'V2';
            adapter.log.info('API version V2 - V2 is not supported anymore');
...
async function ems_read(version) {
    ...
    if (adapter.config.ems_dallas) {
        if (last_version.substring(0, 3) < '3.6') {
            ...
            try {
                ...
                adapter.log.info(`error reading dallassensors:  ${error}`);
                data = 'Invalid';
            }
            await adapter.delay(ems_http_wait);
        ...
        if (sensors > 0) {
            ...
            try {
                ...
                data = 'Invalid';
                adapter.log.info(`error reading temperaturesensors:  ${error}`);
            }
            await adapter.delay(ems_http_wait);
            ...
            if (data != 'Invalid') {
                ...
                try {
                    ...
                    adapter.log.info(`error parsing temperaturesensors:  ${error}`);
                }
    ...
    if (adapter.config.ems_analog) {
        ...
        if (data != 'Invalid') {
            ...
            try {
                ...
                adapter.log.info(`error reading analogsensors:  ${error}`);
            }
    ...
    if (adapter.config.ems_custom_elements) {
        ...
        try {
            ...
            //adapter.log.info(`error reading custom entities:  ${error}`);
            data = 'Invalid';
        }
        await adapter.delay(ems_http_wait);
        ...
        if (adapter.config.statistics) {
            await adapter.setObjectNotExistsAsync('statistics.ems-custom-read', {
                type: 'state',
                common: {
                    type: 'number',
                    name: 'ems read time for polling custom entities',
                    unit: 'seconds',
                    role: 'value',
                    read: true,
                    write: true,
            ...
            adapter.setStateAsync('statistics.ems-custom-read', { ack: true, val: t3 });
        }
    }
...
async function write_custom(statename, value, def) {
    //adapter.log.info("custom elements: "+statename);
    adapter.log.info(JSON.stringify(def));
    const array = statename.split('.');
    const obj = { _id: statename, type: 'state', common: {}, native: {} };
    obj.common.role = 'value';
    obj.common.name = `ems: ${statename}`;
