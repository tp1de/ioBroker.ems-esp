...
const enums = async function(a,id) {
	...
	enum_add(adapter.config.room,"ems-esp."+adapter.instance+"."+id);
	enum_add(adapter.config.function,"ems-esp."+adapter.instance+"."+id);
};
...
async function enum_add(enumName,id) {
	...
	//adapter.log.info(enumn+ "  "+ id + " "+ JSON.stringify(obj));
	if (!found) {
		try {
