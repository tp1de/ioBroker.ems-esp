		...
		if (own_states[i].polling && !unloaded) {
			...
			try {
				for (let ii = 0; ii < 20; ii++) {
					...
					if (type == typer && r != "<empty>" && r != "") {break;	}
				}
			} catch (e) { adapter.log.error(e);}

			if (type != typer) { adapter.log.info(type + " " + typer); }
			else {

				try {
					let resp = JSON.parse(response).response;
					if (resp == "") own_states[i].polling = false; 
				} catch (e) { }
				...
				if (own_states[i].polling) {
					try {
						...
						if (r == "<empty>" || r == "") own_states[i].polling = false;
					}
					catch (error) { adapter.log.error("catch");own_states[i].polling = false; }
				}
				let d;
