...
async function resp(url, urls, init, type, telegram) {
	...
	try {
		...
		for (let ii = 0; ii < loops; ii++) {
			...
			if (ii == 10 || ii == 20 || ii == 30) { await ems_put(urls, telegram); }
		}
	} catch (e) { adapter.log.error(e); }
...
function init_own_states(src) {
	const devices = [
		{
			"state": "thermostat.hc1.switchPrograms.switchmodes",
			"src": src,
			"type": "01B9",
			"offset": "13",
			"polling": true,
		...
		{
			"state": "thermostat.hc2.switchPrograms.switchmodes",
			"src": src,
			"type": "01BA",
			"offset": "13",
			"polling": true,
		...
		{
			"state": "thermostat.hc3.switchPrograms.switchmodes",
			"src": src,
			"type": "01BB",
			"offset": "13",
			"polling": true,
		...
		{
			"state": "thermostat.hc4.switchPrograms.switchmodes",
			"src": src,
			"type": "01BC",
			"offset": "13",
			"polling": true,
