...
const init = async function (a, i) {
	...
	if (count > 0) {
		adapter.log.info("custom entity poll for " + count + " states every 1 minute");
...
async function ems_poll() {
		...
		if (own_states[i].polling && !unloaded) {
			...
			if (own_states[i].polling) {
				...
				switch (own_states[i].state_type) {
					...
					default:
						try {
							adapter.log.info(own_states[i].state+ " " + own_states[i].state_type + " " + r);
						}
						catch (error) { adapter.log.warn("no poll response for custom telegram " + own_states[i].state); }
						break;
				}
	...
	if (adapter.config.statistics) {
		adapter.setObjectNotExists("statistics.ems-own-read",{type: "state",
