...
const init = async function (a, i) {
	...
	// read ems-esp bus id
	try {
		const busid_d = (await adapter.getStateAsync("esp.Settings.ems bus id")).val;
		busid = 0x00 + busid_d.toString(16).toUpperCase();
		...
		let dev = {}, dev_count = 0;
		try {dev = JSON.parse(data).Devices; dev_count = dev.length; }catch(e){}
		
		for (let i=0; i < dev_count; i++) {
			const device = dev[i].type.toLowerCase();
			if (device == "thermostat") thermostat_id = dev[i]["device id"];
		}
		thermostat_id = thermostat_id.replace("0x","");	
	
		let devices = adapter.config.devices;
		if (devices.length == 0) devices = init_own_states(thermostat_id);
		...
		if (count > 0) {
			...
			i.poll = setInterval(function () { ems_poll(); }, 60000); // 1 minutes
		}
} catch(e) {}
};
	...
	for (let i = 0; i < own_states.length; i++) {
		if (own_states[i].polling && !unloaded) {
			...
			if (own_states[i].type.length > 2) {
				...
				if (own_states[i].bytes <10) telegram += "0";
			...
			try {
				for (let ii = 0; ii < 10; ii++) {
					//await adapter.delay(adapter.config.ems_poll_wait);
					...
					} catch (eee) { typer = ""; offsetr = "00"; }
					
					if (type == typer && offset == offsetr) {break;	}
				}
...
async function ems_get(url)  {
	...
	try {
		...
		//if (b.data.message != undefined || b.data == {}) throw new Error("Request failed "+ url);
...
async function ems_put(url, value) {
	try {
		...
		//adapter.log.info(JSON.stringify(r.data));
		return;
	} catch(e) {
...
async function write_ownstate(statename, value, own) {
	...
	if (own.states != "") {
		try {obj.common.states = own.states;obj.common.type = "mixed";}
