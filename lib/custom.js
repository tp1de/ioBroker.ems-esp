...
const init = async function (a, i) {
    ...
    let init = false;
    try {
        busid_d = (await adapter.getStateAsync('esp.Settings.ems bus id')).val;
    } catch {}
    try {
        busid_d = (await adapter.getStateAsync('esp.Settings.ems_bus_id')).val;
    } catch {}
    try {
        busid_d = (await adapter.getStateAsync('esp.settings.emsBusId')).val;
    } catch {}
    ...
    try {
        ...
        for (let i = 0; i < dev_count; i++) {
            ...
            if (device == 'thermostat') {
                thermostat_count += 1;
                try {
                    thermostat_id = dev[i]['device id'];
                } catch (ee) {}
                if (thermostat_id == undefined) {
                    try {
                        thermostat_id = dev[i]['deviceID'];
                    } catch (ee) {}
                }

                if (dev[i]['handlers received'] != undefined) {
                    handlers += `${dev[i]['handlers received']} `;
                }
                if (dev[i]['handlers fetched'] != undefined) {
                    handlers += `${dev[i]['handlers fetched']} `;
                }
                if (dev[i]['handlers pending'] != undefined) {
                    handlers += `${dev[i]['handlers pending']} `;
                }
                if (dev[i]['handlers ignored'] != undefined) {
                    handlers += dev[i]['handlers ignored'];
                }

                if (dev[i]['handlersReceived'] != undefined) {
                    handlers += `${dev[i]['handlersReceived']} `;
                }
                if (dev[i]['handlersFetched'] != undefined) {
                    handlers += `${dev[i]['handlersFetched']} `;
                }
                if (dev[i]['handlersPending'] != undefined) {
                    handlers += `${dev[i]['handlersPending']} `;
                }
                if (dev[i]['handlersIgnored'] != undefined) {
                    handlers += dev[i]['handlersIgnored'];
                }
            }
        }
        ...
        state_suffix = '';
        if (adapter.config.km200_active && adapter.config.km200_structure) {
            state_suffix = '_';
        }
        ...
        if (count > 0 && count < 5) {
            ...
            i.poll = adapter.setInterval(function () {
                ems_poll();
            }, 60000);
        }
        if (count >= 5 && count < 10) {
...
async function ems_poll() {
                ...
                if (type != typer) {
                    ...
                    if (devices[i].polling) {
                        ...
                        switch (devices[i].state_type) {
                            case 'switchPrograms':
                                try {
                                    read_switchPrograms(devices[i], d);
                                } catch (error) {
                                    adapter.log.warn(
                                        `no poll response for extended entity telegram ${devices[i].state}`,
                                    );
                                }
                                break;
                ...
                if (switchTimes[i].typei.trim() != '') {
                    ...
                    try {
                        for (let ii = 0; ii < devices.length; ii++) {
                            if (
                                switchTimes[i].typei == devices[ii].type &&
                                switchTimes[i].offseti == devices[ii].offset
                            ) {
                                st = devices[ii].state;
                                break;
                            }
                    ...
                    if (adapter.config.km200_structure) {
                        st = st.replace('thermostat', 'heatingCircuits');
                    }

                    try {
                        if (st != '') {
                            val = (await adapter.getStateAsync(st)).val;
                        }
                    } catch {}
                }
                ...
                if (type != typer) {
                    ...
                    if (switchTimes[i].polling) {
                        try {
                            ...
                            adapter.log.error('catch telegram');
                            switchTimes[i].polling = false;
                        }
                    }
                    ...
                    if (switchTimes[i].polling) {
                        ...
                        try {
                            read_switchPrograms(own, d);
                        } catch (error) {
                            adapter.log.warn(`no poll response for extended entity telegram ${switchTimes[i].state}`);
                        }
                    }
                }
            }
    ...
    if (adapter.config.statistics) {
        await adapter.setObjectNotExistsAsync('statistics.ems-own-read', {
            ...
            common: {
                type: 'number',
                name: 'ems read time for own states polling',
                unit: 'seconds',
                role: 'value',
                read: true,
                write: true,
...
async function ems_get(url) {
    ...
    try {
        ...
        throw new Error(`ems-get request failed`);
    }
}
...
async function read_holidayModes(devices, d) {
    if (d.length >= 18) {
        ...
        if (value > 0 && value < 5) {
            write_ownstate(`${devices.state + state_suffix}.hcMode`, value, own);
        }
        ...
        if (value > 1 && value < 4) {
            write_ownstate(`${devices.state + state_suffix}.dhwMode`, value, own);
        }
        ...
        if (j1 > 2020) {
            ...
            if (d[17] == 'FF') {
                assignedTo.push('dhw1');
            }
            if (d[18] == 'FF') {
                assignedTo.push('dhw2');
            }
        }
        own.states = '';
    ...
    for (let d = 0; d < 7; d++) {
        for (let ii = 0; ii < 6; ii++) {
            ...
            if (tt[i2] != 'FF') {
                ...
                let t;
                if (m == 0) {
                    t = `${Math.round(min / 60)}:00`;
                } else {
                    t = `${parseInt((min - m) / 60)}:${m}`;
                }
                const sp = {
                    dayOfWeek: day(d),
...
async function write_ownstate(statename, value, own) {
    ...
    if (own.state_type == 'number' && own.enum == '') {
        ...
        if (own.min !== '') {
            obj.common.min = Number(own.min);
        }
        if (own.max !== '') {
            obj.common.max = Number(own.max);
        }
    }

    if (own.state_type == 'string') {
        obj.common.type = own.state_type;
    }
    if (own.state_type == 'hex') {
        obj.common.type = 'string';
    }
    if (own.state_type == 'switchPrograms') {
        obj.common.type = 'json';
    }
    if (own.state_type == 'holidayModes') {
        obj.common.type = 'string';
    }
    if (own.state_type == 'holidayPeriod') {
        obj.common.type = 'string';
    }

    if (typeof own.states == 'object') {
        own.states = JSON.stringify(own.states);
    }

    if (own.states != '' && typeof own.states == 'string') {
        own.states = own.states.replace('{', '');
        own.states = own.states.replace('}', '');
        try {
            obj.common.states = JSON.parse(`{${own.states}}`);
            obj.common.type = 'mixed';
        } catch (e) {
            adapter.log.error(`own.states:${own.states}`);
        }
    }
    ...
    if (own.state_type == 'switchPrograms') {
        obj.common.role = 'switchPrograms';
    }
    if (own.state_type == 'holidayModes') {
        obj.common.role = 'holidayModes';
    }
    if (own.state_type == 'holidayPeriod') {
        obj.common.role = 'holidayModes';
    }
...
const state_change = async function (id, state, obj) {
    ...
    try {
        if (obj.common.role == 'value' && obj.native.ems_state_type == 'number') {
            ...
            if (vc.length == 1 || vc.length == 3) {
                vc = `0${vc}`;
            }
            let type = obj.native.ems_type;

            if (type.substring(0, 2) == '0x') {
    ...
    if (obj.common.role == 'holidayModes') {
        ...
        if (obj.native.ems_state_type == 'holidayPeriod') {
            ...
            try {
                ...
                adapter.log.error('wrong holiday period');
            }
        } else {
            const offset = '09';
            let telegram = `${busid} ${src} FF ${offset} ${type.substring(0, 2)} ${type.substring(2, 4)} `;
            let hex = '00';
            if (value.search('hc1') > -1) {
                hex = 'FF';
            }
            telegram += `${hex} `;
            hex = '00';
            if (value.search('hc2') > -1) {
                hex = 'FF';
            }
            telegram += `${hex} `;
            hex = '00';
            if (value.search('hc3') > -1) {
                hex = 'FF';
            }
            telegram += `${hex} `;
            hex = '00';
            if (value.search('hc4') > -1) {
                hex = 'FF';
            }
            telegram += `${hex} `;
            telegram += '00 00 00 00 ';
            hex = '00';
            if (value.search('dhw1') > -1) {
                hex = 'FF';
            }
            telegram += `${hex} `;
            hex = '00';
            if (value.search('dhw2') > -1) {
                hex = 'FF';
            }
            telegram += hex;
            post(id, telegram);
...
async function post(id, telegram) {
    ...
    try {
        ...
        adapter.log.error('error write change ems-esp raw error');
    }
}
