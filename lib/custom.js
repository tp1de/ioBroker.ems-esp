...
const init = async function(a,i) {
	...
	if (count > 0) adapter.log.info("custom entity poll for " + count + " states every 1 minute");

	i.poll = setInterval(function() {ems_poll();}, 60000); // 1 Minutes


	if (adapter.config.devices.length == 0) {
		let inst = "system.adapter."+adapter.namespace;
		let obj = await adapter.getForeignObjectAsync(inst);
		obj.native.devices = own_states;
		await adapter.setForeignObjectAsync(inst,obj);
		adapter.log.info("ems: custom entities configuration stored .... instance will restart");
	} 
...
async function ems_poll() {
	for (let i=0;i < own_states.length;i++){
		if (own_states[i].polling && !unloaded) {
			...
			let dest = hexValue.toString(16).toUpperCase();
			telegram += dest + " ";
			...
			try {
				for (let ii=0; ii<20; ii++) {
					...
					let typer = "";
					try {typer = type_to_raw(JSON.parse(response).type);} catch(e) {typer="";}
					if (type == typer) {
...
async function read_holidayModes(own_states,d) {
	if (d.length >= 18) {
		...
		write_ownstate(own_states.state+state_suffix+".hcMode",parseInt(d[6],16),own);
		...
		own.min = 2;
		own.max = 3;
		write_ownstate(own_states.state+state_suffix+".dhwMode",parseInt(d[8],16),own);
...
function switchProgram_to_telegram(spa){
	...
	for (let i=0;i < spa.length;i++) {
		...
		data += hex + " ";
		c = c + 1;
		//adapter.log.info(i+"  "+c+ " "+spa[i].dayOfWeek+ " "+ spa[i+1].dayOfWeek);
		...
		if (obj.common.role == "holidayModes") {
			...
			if (obj.native.ems_state_type == "holidayPeriod") {
				...
				if (value.length < 21) {  // reset holiday period
					...
					adapter.setState(obj.common.id, {ack: true, val: value});
				}
				const offset = "00";
